...
namespace MP1
{
    public class PolynomialCalculus
    {
        ...
        {
            double derivative = 0;//set derivative value to 0
            
            for (int i = 0; i < coefficientList.Count; i++)//goes through each value in the coefficent list
            {
                derivative += coefficientList[i] * i * Math.Pow(x, i - 1);//calculates each derivative in the polynomial seperatly and adds all together
            }
